...
#undef EXIT_WITH_MSG
  ...
  void Run() {
    ...
    while (running() && !interrupt_received) {
      ...
      if((horizontal_position_ % current_image_.width)==0) {
        // I believe this is where we could drop in some code to signal that we have scrolled off the screen
        printf("End of scrolling image off screen?");
      }
